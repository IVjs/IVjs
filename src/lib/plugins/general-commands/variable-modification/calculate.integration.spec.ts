...
describe('.calculate() integration', () => {
  ...
  test('modifies variable in place if no `storeIn` is provided', async () => {
    iv.node('anything').calculate({ var: 'count', add: 1 });
  ...
  test('stores the calculation in a different variable if `storeIn` is provided', async () => {
    iv.node('anything').calculate({ var: 'count', add: 1, storeIn: 'newCount' });
  ...
  test('subtract', async () => {
    iv.node('anything').calculate({ var: 'count', subtract: 1 });
  ...
  test('multiply', async () => {
    iv.node('anything').calculate({ var: 'count', multiply: 10 });
  ...
  describe('when resolved variable is not a number', () => {
    ...
    describe.skip('capturing nested error...', () => {
      // cannot reassign error to observe
  ...
  test('it throws with options object with unexpected properties', async () => {
    const setup = () => iv.node('anything').calculate({ var: 'count', make: 1, storeIn: 'count' } as any);

    expect(setup).toThrow();
  });

  test('it throws for options object without "var" prop', async () => {
    const setup = () => iv.node('anything').calculate({ storeIn: 'count', add: 1 } as any);

    expect(setup).toThrow();
  });

  test('it throws for options object without an operation', async () => {
    const setup = () => iv.node('anything').calculate({ var: 'count', storeIn: 'something' } as any);

    expect(setup).toThrow();
  });

  test('it throws for options object with more than one oepration', async () => {
    const setup = () => iv.node('anything').calculate({ var: 'count', add: 1, subtract: 1 });
