...
type ifOptions = Partial<Is & IsGreaterThan & IsLessThan & IsGreaterThanOrEqualTo & IsLessThanOrEqualTo & IsBetween>;
...
type NodeExtended = { [P in keyof NodeExtensions]: ReplaceReturnType<NodeExtensions[P], IvNode> };
...
export class Node implements BaseNode {
  ...
  private pusher(command: ICommand.AnyCommand[] | ICommand.AnyCommand) {
    if (Array.isArray(command)) {
      return command.forEach(c => this.pusher(c));
    }
  ...
  public if(optionsObj: ifOptions): IvNode {
    ...
    if (optionsObj.is) {
      ...
      this.switchDo.do.push({
        ...
        isGreaterThanOrEqualTo: optionsObj.isGreaterThanOrEqualTo,
        commands: [],
      });
    } else if (optionsObj.isLessThanOrEqualTo) {
      this.switchDo.do.push({
        varName: optionsObj.var,
        isGreaterThanOrEqualTo: optionsObj.isLessThanOrEqualTo,
        commands: [],
      });
    }
